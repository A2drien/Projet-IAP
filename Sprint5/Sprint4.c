...
void afficher_classement(const TournoisWTA* listeTournois) {
    ...
    for (unsigned int i = 0; i < idxT * nbJoueusesTournoi; i++) {
    ...
    for (unsigned int i = 0; i < idxT * nbJoueusesTournoi; i++) {
        ...
        strcpy(nomJoueuseMaximum, tableauJoueuse[indexMaximum].nomJoueuse);

        for (unsigned int j = i + 1; j < idxT * nbJoueusesTournoi; j++) {
